...
Deployment: https://arun-private-intelligence.arun-private-intelligence.workers.dev/

Current Access policy protects **all traffic** and requires a Cloudflare account member to sign in. The Worker independently permits only `arun.b.bhat@gmail.com`. The owner can use their existing Google sign-in to Cloudflare. Other accounts, including future Cloudflare account members, are rejected by the Worker unless explicitly allowlisted.
...
Cloudflare Access supplies the login page using the Cloudflare account policy. The Worker also validates the Access token's RS256 signature, issuer, application audience, expiry, identity, and application-token type, then checks the email allowlist. Missing configuration denies all access. `run_worker_first: true` protects every asset and API path, including direct Worker URLs. Preview URLs are disabled. No client-only password gate, public report export, browser storage, analytics, or report/token logging is used.
...
5. For owner access, choose **Cloudflare account** as the authentication policy. The Worker's exact email allowlist provides the additional restriction. Alternatively, in Cloudflare One configure **One-time PIN** and an **Allow** policy whose **Emails** include only the owner's address. Do not use “Everyone” or the `gmail.com` domain. New accounts may require adding One-time PIN explicitly.
...
To admit another person later, use a specific-email Access Allow policy with a login provider such as One-time PIN, and add their email to the Worker's `ALLOWED_EMAILS`, then redeploy. Do not grant Cloudflare account membership just to share reports. Revoke a user's Access session when removing access. No deployment workflow is enabled automatically; publishing requires your authenticated Cloudflare session.
