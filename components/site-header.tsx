    ...
    <header className="site-header">
      <div className="header-inner wrap">
        <Link className="brand" href="/" aria-label={`${profile.name} home`}>
          ...
          <span className="brand-name">{profile.name}<span>{profile.role.toUpperCase()}</span></span>
        </Link>
        <nav className={menuOpen ? "main-nav is-open" : "main-nav"} aria-label="Main navigation">
          ...
          <a href={profile.resume} target="_blank" rel="noopener noreferrer" aria-label="Resume (PDF, opens in a new tab)" onClick={() => setMenuOpen(false)}>Resume</a>
