...
function outputFor(command: string): string {
  switch (command) {
    ...
    case "whoami": case "about": case "cat /etc/profile": return `${profile.name}\n${profile.role} · ${profile.location}\n\nI build and run streaming infrastructure with Kafka, Kubernetes,\nand Terraform, and write the tools that make it easier to operate.\n\nAsk me about Kafka internals, consumer lag, Schema Registry,\nor writing your own Terraform provider.`;
    case "skills": case "cat skills.md": return skillGroups.map((group) => `${group.title.toUpperCase()}\n  ${group.skills.map((skill) => skill.name).join(" · ")}`).join("\n\n");
